...
from optimizer_loam import Optimizer_Loam
import utils
...
class Mapping_Loam:
    ...
    def append_undistorted(self, pcd, T, points, vis=False):
        self.cnt += 1
        edge_points, surface_points = points
        ...
        if not self.init:
            ...
            #self.aligned_pcds.append(utils.get_pcd_from_numpy(pcd))
            self.all_edges = utils.get_pcd_from_numpy(np.vstack(edge_points))
            self.all_surfaces = utils.get_pcd_from_numpy(np.vstack(surface_points))
        else:
            prior_position = T @ self.position
            edge_points = np.asarray(self.filter_pcd(utils.get_pcd_from_numpy(np.vstack(edge_points)), 'edge').points)
            surface_points = np.asarray(self.filter_pcd(utils.get_pcd_from_numpy(np.vstack(surface_points)), 'surface').points)
            ...
            if len(edges) > 0 and len(surfaces) > 0:
                optimizer = Optimizer_Loam((np.vstack(edges), np.vstack(edge_A), np.vstack(edge_B)),
                                          (np.vstack(surfaces), np.vstack(surface_A), np.vstack(surface_B)))
                T = optimizer.optimize_2()
                self.position = T.T() @ prior_position
                self.aligned_pcds.append(utils.get_pcd_from_numpy(T.transform_array(transformed_pcd)))
                if self.cnt % 3 == 0:
                    self.all_edges += utils.get_pcd_from_numpy(T.transform_array(transformed_edge_points))
                    self.all_surfaces += utils.get_pcd_from_numpy(T.transform_array(transformed_surface_points))
                    self.all_edges = self.filter_pcd(self.all_edges, 'edge')
                    self.all_surfaces = self.filter_pcd(self.all_surfaces, 'surface')
            else:
                self.aligned_pcds.append(utils.get_pcd_from_numpy(transformed_pcd))
                self.all_edges += utils.get_pcd_from_numpy(transformed_edge_points)
                self.all_surfaces += utils.get_pcd_from_numpy(transformed_surface_points)


            if vis:
                #o3d.visualization.draw_geometries(self.aligned_pcds)
                #self.vis.update_geometry(self.aligned_pcds)
                self.vis.add_geometry(self.aligned_pcds[-1])
                self.vis.poll_events()
                self.vis.update_renderer()
                
        return self.aligned_pcds[-1]
