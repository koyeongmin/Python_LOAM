...
class Odometry_Loam:
    ...
    def predict(self, lidar_data, feature):
        sharp_points, less_sharp_points, flat_points, less_flat_points = feature

        T = None

        if not self.inited: # at an initiat point
            self.inited = True
            T = np.zeros(6)
        else:
            ...
            surface_corresp = self.find_surface_correspondences(flat_points, lidar_data)
            correspondences_time = time.time()
            print("correspondences_time = ", correspondences_time-start)

            #opimization, find transformation between lidar data
            optimizer = Optimizer_Loam(edge_corresp, surface_corresp)
            T = optimizer.optimize()
            op_time = time.time()
            print("op_time = ", op_time-correspondences_time)

            #show surface feature points
            '''
            ...
            pcd = utils.get_pcd_from_numpy(mrob.geometry.SE3(T).transform_array(lidar_data[0]))
        ...
        x = utils.get_pcd_from_numpy(np.vstack(less_flat_points))
        ...
        return mrob.geometry.SE3(T).T(), (self.last_less_sharp_points, self.last_less_flat_points)
    ...
    def find_edge_correspondences(self, sharp_points):
        ...
        edge_1 = []
        edge_2 = []
        less_sharp_points_tree = o3d.geometry.KDTreeFlann(utils.get_pcd_from_numpy(self.last_less_sharp_points))
        for i in range(corners_cnt):
            ...
            if dist[0] < self.DISTANCE_SQ_THRESHOLD:
                ...
                dist_to_sel_point = utils.matrix_row_wise_dot_product((self.last_less_sharp_points[:, :3] - point_sel[:3]),
    ...
    def find_surface_correspondences(self, flat_points, pcd):
        ...
        less_flat_points_tree = o3d.geometry.KDTreeFlann(utils.get_pcd_from_numpy(self.last_less_flat_points))
        for i in range(surface_cnt):
            ...
            dist_to_sel_point = utils.matrix_row_wise_dot_product((self.last_less_flat_points[:, :3] - point_sel[:3]),
